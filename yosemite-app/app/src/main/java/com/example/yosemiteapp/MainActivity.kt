package com.example.yosemiteapp
...
class MainActivity : AppCompatActivity(), SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    private final var permissions = arrayOf(Manifest.permission.CAMERA)
    private final var PERMISION_OK = 0;
    ...
    private fun checkPermissions() {
        for (permission in permissions) {
            ...
            {
                ActivityCompat.requestPermissions(this, permissions, PERMISION_OK)
                return
            }
        }
    }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (requestCode == PERMISION_OK && grantResults[0] == PackageManager.PERMISSION_GRANTED) startCam()
    }
...
class MainActivity : AppCompatActivity(), SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    private val neededPermissions = arrayOf(CAMERA, WRITE_EXTERNAL_STORAGE, INTERNET)
    ...
    private fun imgToServer(bytes: ByteArray) {
        var url_text = findViewById<EditText>(R.id.serverEndpointTxt).text.toString()
        println(url_text)
        if (!URLUtil.isValidUrl(url_text)) {
            Toast.makeText(this@MainActivity, "Invalid Endpoint!", Toast.LENGTH_LONG).show()
            return
        }
        var result_ok = false
        Thread {
            var endpoint = URL(url_text)
